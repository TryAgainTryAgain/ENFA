...
compareRaster(c(elevation,tri,slope,tmin01,tmax07,ppt1005,ppt0609,snow,ndvimeanmax,ndvicv,aspen,conifer,meadow,mixed,rock,shrub))
...
AvailProjArea <-extract(MarmotStack, RandPtsProj)
str(AvailProjArea)
head(AvailProjArea)
...
#writeRaster(HabitatVariablesMarfla, "SNV/HabitatVariablesMarfla")
#HabitatVariablesMarfla <- raster("SNV/HabitatVariablesMarfla")
...
#Get them into a CSV? 
write.csv(habvarsMarfla, file = "HabitatVariablesMarfla.csv")
...
elevfilter <- Marflahabvars.temp$Elevation > 2500
...
##HELP - Stop Here - I think the problem is in the code in the 20 lines below.  

#There is a problem combining the available (AvailProjArea) and the used(Marflahabvars).  I think it is because the Available is a matrix, the used is a data.frame, and I haven't added the coordinates like I did for the used - resulting in different column numbers.  So attempting to use the code to add UTM to the available.

AvailProjArea.dfm <- data.frame(AvailProjArea)
str(AvailProjArea.dfm)
colnames(AvailProjArea.dfm) <- var.names
str(AvailProjArea.dfm)

Avail.coords <- data.frame(coordinates(RandPtsProj))
colnames(Avail.coords) <- c("UTMEW", "UTMNS")
str(Avail.coords)
nrow(unique(Avail.coords))
head(Avail.coords)

habvarsAvailProj <- cbind(Avail.coords, AvailProjArea.dfm)
head(habvarsAvailProj)
#plot(habvarsAvailProj) - this plots something weird, then makes R freeze and crash
#There is something wrong above.  Because this plots a bunch of 

# Combine used and available habitat 
Marflahabvars.temp3 <- rbind(Marflahabvars.temp2, habvarsAvailProj)
...
##STARTHERE
##HELP - now all of the correlations are showing up as NA.  What broke??  
